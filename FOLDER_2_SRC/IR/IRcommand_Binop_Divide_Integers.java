...
public class IRcommand_Binop_Divide_Integers extends IRcommand
{
	...
	public void MIPSme()
	{
		...
		String label_division_by_zero    = getFreshLabel("division_by_zero");
		String label_legal_division    = getFreshLabel("legal_division");
		...
		sir_MIPS_a_lot.getInstance().beq(t2,null,label_division_by_zero,true);
		sir_MIPS_a_lot.getInstance().jump(label_legal_division);
		...
		/*                    */
		/**********************/
		sir_MIPS_a_lot.getInstance().label(label_division_by_zero);
		sir_MIPS_a_lot.getInstance().printRuntimeErrorMsg("string_illegal_div_by_0");
		sir_MIPS_a_lot.getInstance().exitProgram();
		
		//if we got here - this is legal division
		
		sir_MIPS_a_lot.getInstance().label(label_legal_division);
		/*********************/
		/* [4] t4 := t1 + t2 */
		/*********************/
		...
		sir_MIPS_a_lot.getInstance().blt(t1_div_t2,intMin,label_neg_overflow);
		sir_MIPS_a_lot.getInstance().blt(intMax,t1_div_t2,label_overflow);
		sir_MIPS_a_lot.getInstance().bge(intMax,t1_div_t2,label_no_overflow);
